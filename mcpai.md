...
Info
Ephemeral MCP servers require the "Allow per-request MCPs" setting to be enabled in .
...
MCP servers can be pre-configured in your mcp.json file. This is the recommended approach for using MCP servers that take actions on your computer (like ) and servers that you use frequently.

Info
MCP servers from mcp.json require the "Allow calling servers from mcp.json" setting to be enabled in .
